...
var arrNum: number[] = [3, 4, 5, 3]
console.log(arr[0], arrNum[1])

// Array实例
var arr_num: number[] = new Array(4)
for (var i = 0; i < arr_num.length; i++) {
  arr_num[i] = i + 5
  console.log(arr_num[i])
}

var arr_str: string[] = new Array('af', 'aq', 'rd', 'af')
...
// 多维数组
var arr2: number[][] = [[1, 2, 3], [5, 6, 8], [8, 6, 4]]
console.log(arr2[0][2])
console.log(arr2[2][0])

// 数组方法
console.log('concat 连接两个或更多的数组，并返回结果：' + arr.concat(arr_str))
console.log('every 检测数值元素的每个元素是否都符合条件：' + arrNum.every((i) => { return i > 4 }))
console.log('filter 检测数值元素，并返回符合条件所有元素的数组：' + arrNum.filter((i) => { return i > 3 }))
console.log('forEach 数组每个元素都执行一次回调函数：' + arrNum.forEach((n) => {console.log(n)}))
...
console.log('map 通过指定函数处理每个元素：' + arrNum.map((i) => {i ++; console.log(i)}))
console.log('some 检测数组元素中是否有元素符合指定条件：' + arrNum.some((i) => {return i == 5}))
